...
For the **BACnet B-ASC (Application Specific Controller) Node.js example** -
...
> `CHANGE ALL OF THIS BEFORE YOU SHIP` block at the top of `main.ts`. The
...
| **Product Name** | CAS BACnet Stack B-ASC Example |
| **Product Model Number** | CAS BACnet Stack B-ASC Example |
| **Application Software Version** | 1.0.0 |
| **Firmware Revision** | 1.0.0 |
| **BACnet Protocol Version** | 1 |
| **BACnet Protocol Revision** | 24 (`BACNET_PROTOCOL_REVISION` in the stack's `CASBACnetStackOptions.h` at the pinned commit; the example's start-up banner does not print it - read it back with ReadProperty to confirm) |

**Product Description:** a minimal BACnet/IP device built on the CAS BACnet
Stack's Node.js/TypeScript package. It presents three read-only input objects
and three commandable output objects (Analog, Binary, Multi-State), answers
ReadProperty and WriteProperty for every required property of every object,
accepts DeviceCommunicationControl, and is discoverable by Who-Is / I-Am and
Who-Has / I-Have. It is a tutorial for implementers of the B-ASC profile.
...
No object is dynamically creatable or deletable. `Present_Value` is writable
on the three output objects only; every other object and property is
read-only.
...
### Analog Output 1 "Chartreuse" - commandable REAL, percent; 16-slot priority array, Relinquish_Default 0.0; the application resolves the priority array itself (CommandRead/CommandWrite/CommandRelinquish) rather than relying on the stack's own commandable support, so Present_Value and Priority_Array are served by the app, not generated
...
### Binary Output 1 "Fuchsia" - commandable active/inactive; 16-slot priority array, Relinquish_Default inactive; same application-owned priority array as Analog Output 1
...
### Multi-state Output 1 "Indigo" - commandable state 1..3, validated on write; 16-slot priority array, Relinquish_Default state 1; same application-owned priority array as Analog Output 1
